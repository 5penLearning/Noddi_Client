...
function AppLayout() {
  ...
  const navigateFromNotification = (notification) => {
    ...
    if (navigation.type === 'PROJECT') {
      navigate(
        `/projects/${navigation.projectId ?? navigation.referenceId}`,
      );
      return;
    }
  ...
  const handleVerifyPassword = async (password) => {
    if (!profile?.email) {
      setPasswordConfirmError('사용자 이메일 정보를 확인할 수 없습니다.');
      return;
    }
    ...
    try {
      ...
      const verifiedUserId = response?.result?.userId;

      if (
        verifiedUserId &&
        Number(verifiedUserId) !== Number(profile.userId)
      ) {
  ...
  return (
    <div className="relative flex h-screen overflow-hidden bg-[#FAFFFC] p-[10px]">
      <div
        className={`hidden h-full shrink-0 overflow-hidden transition-[width,opacity] duration-200 ease-out lg:block ${isDesktopSidebarOpen ? 'w-16 opacity-100' : 'w-0 opacity-0'
          }`}
      >
        <SidebarNavigation
      ...
      <main
        className={`flex min-w-0 flex-1 flex-col transition-[margin] duration-200 ease-out ${isDesktopSidebarOpen ? 'lg:ml-[10px]' : 'lg:ml-0'
          } ${isHomePage
            ? 'gap-[10px] rounded-[20px] bg-[var(--color-gray-100)] p-[10px]'
            : 'gap-[10px]'
          }`}
      >
        <header className="flex h-14 w-full shrink-0 items-center justify-between">
          <div className="flex min-w-0 items-center gap-2">
            ...
            <button
              ...
              aria-label={
                isDesktopSidebarOpen ? '사이드바 접기' : '사이드바 펼치기'
              }
      ...
      <PasswordConfirmModal
        ...
        onClose={handleClosePasswordConfirm}

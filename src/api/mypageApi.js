...
export const updateMyPassword = async ({
  ...
  const { data } = await api.patch('/api/v1/users/me/password', {
    currentPassword,
    newPassword,
  });
...
export const getProjectInvitations = async () => {
  const { data } = await api.get('/api/v1/projects/invitations');

  return data;
};

export const respondProjectInvitation = async (
  inviteId,
  isAccepted,
) => {
  const { data } = await api.post(
...
export const getTeamInvitations = async () => {
  const { data } = await api.get('/api/v1/users/me/teams/invitations');

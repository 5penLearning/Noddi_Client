    ...
    <div className="rounded-xl border border-[#DCE3E0] bg-[#FAFBFB] p-4">
      <div>
        ...
        <textarea
          ...
          className="w-full resize-none rounded-lg border border-[#DCE3E0] bg-white px-3 py-2.5 text-sm leading-6 text-[#303633] outline-none transition focus:border-[#31F5A0]"
        />
      </div>
      ...
      <div className="mt-4">
        ...
        <select
          ...
          onChange={onChange}
          className="h-10 w-full rounded-lg border border-[#DCE3E0] bg-white px-3 text-sm text-[#303633] outline-none transition focus:border-[#31F5A0]"
        >
          <option value="">
            담당자 없음
          </option>
...
function ActionItemPanel({
    ...
    try {
      ...
      await createActionItem(
        meetingId,
        {
          content,
          assigneeUserId:
            creatingForm.assigneeUserId
              ? Number(
                creatingForm.assigneeUserId,
              )
              : null,
          dueDate:
            creatingForm.dueDate ||
            null,
        },
      );
  ...
  return (
    <section className="sticky top-6 rounded-2xl border border-[#E5EAE8] bg-white p-5">
      ...
      {isMembersLoading && (
        <p className="mb-3 text-xs text-[#8A9490]">
          팀원 정보를 불러오고
          있습니다.
        </p>
      )}
      ...
      {actionItems.length ===
        ...
        !isCreateOpen && (
          <div className="rounded-xl bg-[#F7F9F8] px-4 py-8 text-center">
            <p className="text-sm font-medium text-[#59625F]">
              등록된 할 일이
              없습니다.
            </p>
                ...
                >
                  <div className="flex items-start justify-between gap-2">
                    <div className="flex min-w-0 items-start gap-2">
                      ...
                      <p
                        className={`text-sm font-medium leading-6 ${item.status ===
                          'COMPLETED'
